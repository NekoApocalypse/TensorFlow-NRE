...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        self.input_y = tf.placeholder(dtype=tf.float32,
                                      shape=[None, num_classes], name='input_y')
        self.total_shape = tf.placeholder(dtype=tf.int32, shape=[big_num + 1],
                                          name='total_shape')
        ...
        if is_training and settings.keep_prob < 1:
            gru_cell_forward = tf.nn.rnn_cell.DropoutWrapper(
                gru_cell_forward,
                output_keep_prob=settings.keep_prob
            )
        ...
        cell_forward = tf.nn.rnn_cell.MultiRNNCell(
            [gru_cell_forward] * settings.num_layers)
        cell_backward = tf.nn.rnn_cell.MultiRNNCell(
            [gru_cell_backward] * settings.num_layers)
        # possible bug? input dimension of first layer is different from
        # other layers
        ...
        self._initial_state_forward = cell_forward.zero_state(total_num,
                                                              tf.float32)
        self._initial_state_backward = cell_backward.zero_state(total_num,
                                                                tf.float32)

        # embedding layer
        inputs_forward = tf.concat([
        ...
        # Notice that entity information is not included in this algorithm

        '''
        # *** IMPORTANT *** behavior of tf.reverse has changed
        inputs_backward = tf.concat(2, [tf.nn.embedding_lookup(word_embedding,
                                                               tf.reverse(
                                                                   self.input_word,
                                                                   [False,
                                                                    True])),
        ...
        with tf.variable_scope('GRU_FORWARD'):
            for step in range(num_steps):
                ...
                (cell_output_forward, state_forward) = cell_forward(
                    inputs_forward[:, step, :], state_forward)
        ...
        with tf.variable_scope('GRU_BACKWARD'):
            for step in range(num_steps):
                if step > 0:
                    tf.get_variable_scope().reuse_variables()
                (cell_output_backward, state_backward) = cell_backward(
                    inputs_backward[:, step, :], state_backward)
                outputs_backward.append(cell_output_backward)

        output_forward = tf.reshape(tf.concat(outputs_forward, 1),
                                    [total_num, num_steps, gru_size])
        output_backward = tf.reverse(tf.reshape(tf.concat(outputs_backward, 1),
                                                [total_num, num_steps,
                                                 gru_size]), [1])
        # * It is possible to use tf.nn.dynamic_rnn?
        # stacked RNN cell dimension problem?

        # word-level attention layer
        '''
        output_h = tf.add(output_forward, output_backward)
        attention_r = tf.reshape(tf.batch_matmul(tf.reshape(tf.nn.softmax(
            tf.reshape(tf.matmul(tf.reshape(tf.tanh(output_h),
                                            [total_num * num_steps, gru_size]),
                                 attention_w), [total_num, num_steps])),
                                                            [total_num, 1,
                                                             num_steps]),
                                                 output_h),
                                 [total_num, gru_size])
        # *** IMPORTANT *** batch_matul is removed in recent versions of
        # *** IMPORTANT *** TensorFLow. Use matmul instead.
        '''
        # softmax(tanh(output_h) * attention_w) .* output_h
        # M = tanh(output_h)
        # A = softmax(W_a * output_h)
        # h* = A .* output_h
        output_h = tf.add(output_forward, output_backward)
        ...
        for i in range(big_num):
            sen_repre.append(tf.tanh(
                attention_r[self.total_shape[i]:self.total_shape[i + 1]]))
            batch_size = self.total_shape[i + 1] - self.total_shape[i]

            sen_alpha.append(tf.reshape(tf.nn.softmax(
                tf.reshape(tf.matmul(tf.multiply(sen_repre[i], sen_a), sen_r),
                           [batch_size])), [1, batch_size]))
            # tf.mul was renamed tf.multiply in recent version of TF
            sen_s.append(tf.reshape(tf.matmul(sen_alpha[i], sen_repre[i]),
                                    [gru_size, 1]))
            # context vector
            sen_out.append(tf.add(
                tf.reshape(tf.matmul(relation_embedding, sen_s[i]),
                           [self.num_classes]), sen_d))
            # * Context vector is multiplied with rel_emb to produce prediction
            ...
            with tf.name_scope("accuracy"):
                self.accuracy.append(tf.reduce_mean(tf.cast(
