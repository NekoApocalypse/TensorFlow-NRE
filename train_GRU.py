...
# tf.app.flags.DEFINE_string('wechat_name', 'Tang-24-0325','the user you want
#  to send info to')
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None,
                ...
                m = network.GRU(is_training=True, word_embeddings=word_embedding,
                                settings=settings)
            global_step = tf.Variable(0, name="global_step", trainable=False)
            optimizer = tf.train.AdamOptimizer(0.001)

            # train_op=optimizer.minimize(m.total_loss,
            ...
            # it's not available in tf 0.11,(because there is no embedding
            # panel in 0.11's TensorBoard) so I delete it =.=
            # you can try it on 0.12 or higher versions but maybe you should
            # change some function name at first.

            # summary_embed_writer = tf.train.SummaryWriter('./model',
            # sess.graph)
            ...
            for one_epoch in range(settings.num_epochs):
                if itchat_run:
                    itchat.send('epoch ' + str(one_epoch) + ' starts!',
                                FLAGS.wechat_name)

                temp_order = np.arange(len(train_word))
                ...
                for i in range(int(len(temp_order) / float(settings.big_num))):
                    ...
                    temp_input = temp_order[
                        i * settings.big_num:(i + 1) * settings.big_num
                    ]
                    for k in temp_input:
